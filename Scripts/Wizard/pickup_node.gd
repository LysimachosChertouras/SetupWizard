...
## NEW: Public helper to allow other scripts (like the Player) to check if input is captured
...
func try_pick_up_nearby_item(is_secondary_action: bool) -> bool:
	...
	var target_item = PhysicsUtils.get_item_under_point(get_world_2d(), mouse_pos, nearby_items)
			
	if target_item and target_item is CodeBlock:
		# Check if the block is a "Source" (Infinite or Stack) or a "Single Item"
		var is_source = target_item.stack_count == -1 or target_item.stack_count > 1
		...
		if is_secondary_action:
			...
			if not (target_item.token_data and target_item.token_data.is_writable): return false
			should_type = true
		else:
			should_type = not in_grid
		
		var item_to_hold = target_item
		
		# If it's a source, spawn a copy and update the original's count
		if is_source:
			item_to_hold = target_item.spawn_copy()
			# Spawn the copy in the same container as the player
			player.get_parent().add_child(item_to_hold)
			target_item.decrement_stack()
		
		# Add to Inventory
		if inventory.add_item(item_to_hold):
			item_to_hold.clear_linked_structure()
			
			if should_type and item_to_hold.token_data and item_to_hold.token_data.is_writable:
				typing_focus_active = true
				inventory.set_paused(true)
			
			# If we picked up the actual physical block (not a source copy), 
			# we stop tracking it on the floor
			if not is_source:
			...
			# Inventory full: Clean up the spawned copy
			if is_source: item_to_hold.queue_free()
...
func get_drop_zone_at_position(point: Vector2) -> DropZone:
	...
	for result in results:
		if result.collider is DropZone: return result.collider
	return null

func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.pressed:
		if event.button_index == MOUSE_BUTTON_WHEEL_UP: inventory.cycle_items(1)
		elif event.button_index == MOUSE_BUTTON_WHEEL_DOWN: inventory.cycle_items(-1)
	...
	if event is InputEventKey and event.pressed:
		...
		if event.keycode == KEY_DELETE and active_item:
			if typing_focus_active:
				typing_focus_active = false
				inventory.set_paused(false)
			if active_item is CodeBlock: active_item.clear_linked_structure()
